...
Odśwież text_search + przebuduj indeksy HNSW / GIN / BM25.
  ...
  python rebuild_indexes_v2.py
  python rebuild_indexes_v2.py --skip-refresh
"""
...
from v2.db import rebuild_search_indexes, refresh_child_text_search

logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
logger = logging.getLogger("rebuild_indexes_v2")


def main() -> int:
    parser = argparse.ArgumentParser(description="Rebuild indeksów wyszukiwania v2")
    parser.add_argument(
        "--skip-refresh",
        action="store_true",
